...
# define the reusable bcrypt context
...
class _BcryptCommon(uh.SubclassBackendMixin, uh.TruncateMixin, uh.HasManyIdents,
    ...
    @classmethod
    def _finalize_backend_mixin(cls, mixin_cls=None, backend=None, dryrun=False):
        """
        Modernized helper for compatibility with legacy calls.
        Drops any bcryptor-specific handling.
        """

        # sanity check: optional if you keep old mixin map
        if mixin_cls is not None and backend is not None:
            assert mixin_cls is getattr(cls, "_backend_mixin_map", {}).get(backend, None), \
                "_finalize_backend_mixin() invoked from wrong class"

        if getattr(cls, "_workarounds_initialized", False):
            return True

        verify = cls.verify_password  # modern verify wrapper

        err_types = (ValueError, uh.exc.MissingBackendError)

        def safe_verify(secret, hash):
            """
            Wrap verify() to trap common errors.
            In modern setup, no _bcryptor errors remain.
            """
            try:
        ...
        cls._workarounds_initialized = True

        # expose safe_verify for legacy usage if needed
        cls.safe_verify = staticmethod(safe_verify)
...
class BcryptBackend:
    """
    Modern replacement for Passlib's deprecated _BcryptorBackend.
    Uses passlib[bcrypt] internally.
    """
    ...
    def verify_password(cls, password: str, hashed: str) -> bool:
        ...
        except (ValueError, PasslibSecurityError, PasslibHashWarning, PasslibSecurityWarning) as e:
            log.warning("Error verifying password: %s", e)
            return False

    @classmethod
    def needs_rehash(cls, hashed: str) -> bool:
        """
        Check if a stored hash needs rehashing (e.g., new bcrypt cost).
        """
        return pwd_context.needs_update(hashed)


# -----------------------------------------------------------------------------
# Optional: Warning about deprecated bcryptor (for backwards compatibility)
# -----------------------------------------------------------------------------
def load_backend_bcryptor_warning():
    warn(
        "Support for `bcryptor` is deprecated and removed. "
        "Please use `passlib[bcrypt]` instead.",
        DeprecationWarning,
    )
# -----------------------------------------------------------------------------
...
class bcrypt(_NoBackend, _BcryptCommon):
    ...
    _backend_mixin_map = {
        ...
        #"bcryptor": _BcryptorBackend, #removed
